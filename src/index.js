...
async function MdevStartBot() {
  ...
  mdevbot.ev.on("messages.upsert", async ({ messages }) => {
    ...
    const enviarImagemDePasta = async (rotaDoAquirvo, legenda = "") => {
      await mdevbot.sendMessage(
        from,
        { image: fs.readFileSync(rotaDoAquirvo), caption: legenda },
        { quoted: msg }
      );
    };
    ...
    // comandos
    if (isCommand) {
      const utils = { enviar, enviarImagemViaURL, enviarImagemDePasta };
      await handleCommand(command, args, msg, mdevbot, utils);
    }
  });
}
